...
namespace KOTORModSync.Core.CLI
{
	...
	public static class ModBuildConverter
	{
		...
		{
			MainConfig config = new MainConfig { debugLogging = verbose };
		}
		...
		private static void LogAllErrors( DownloadCacheService downloadCache, bool forceConsoleOutput = false )
		{
			...
			if (hasOtherErrors)
			{
				Dictionary<ErrorCollector.ErrorCategory, List<ErrorCollector.ErrorInfo>> errorsByCategory = _errorCollector.GetErrorsByCategory();
			...
			{
				List<DownloadCacheService.DownloadFailureInfo> failures = downloadCache.GetFailures();
			...
			try
			{
...

				{
					...
					try
					{
						...
						try
						{
							List<DownloadCacheService.DownloadCacheEntry> results = await downloadCache.ResolveOrDownloadAsync(
								component,
...
.ConfigureAwait( false );
		...
		private static async Task<int> RunCacheStatsAsync( CacheStatsOptions opts )
		{
			try
			{
				...
				{
					Console.WriteLine( "=== Distributed Cache Statistics ===" );
					Console.WriteLine( $"Total Resources: {stats.TotalResources}" );
					Console.WriteLine( $"Total Cache Size: {FormatBytes( stats.TotalSize )}" );
					Console.WriteLine( $"Blocked ContentIds: {stats.BlockedContentIds}" );
					Console.WriteLine( $"Last Updated: {stats.LastUpdated:yyyy-MM-dd HH:mm:ss} UTC" );
					Console.WriteLine();
					Console.WriteLine( "Provider Breakdown:" );
					foreach (KeyValuePair<string, int> provider in stats.Providers)
					{
						Console.WriteLine( $"  {provider.Key}: {provider.Value} resources" );
					}
				}
		...
		private static async Task<int> RunCacheClearAsync( CacheClearOptions opts )
		{
			try
			{
				...
				if (!opts.Force)
				{
					Console.Write( "Are you sure you want to clear the distributed cache? (y/N): " );
				...
				Console.WriteLine( $"✓ Cache cleared{(string.IsNullOrEmpty( opts.Provider ) ? "" : $" for provider: {opts.Provider}")}" );
				return 0;
			}
			catch (Exception ex)
			{
		...
		private static async Task<int> RunCacheBlockAsync( CacheBlockOptions opts )
		{
			try
			{
				...
				{
					Console.WriteLine( "Error: ContentId must be exactly 40 hexadecimal characters" );
					return 1;
				}

				DownloadCacheOptimizer.BlockContentId( opts.ContentId, opts.Reason ?? "Manual block via CLI" );
				Console.WriteLine( $"✓ ContentId {opts.ContentId} has been blocked from distributed cache" );
				if (!string.IsNullOrEmpty( opts.Reason ))
				{
					Console.WriteLine( $"  Reason: {opts.Reason}" );
				}

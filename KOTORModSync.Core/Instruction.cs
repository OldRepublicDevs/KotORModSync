	...
	public sealed class Instruction : INotifyPropertyChanged
	{
		...
		public ActionType Action
		{
			...
			set
			{
				...
				// Auto-populate Source, Destination, and Overwrite for new instructions only.
				// "New" means all three fields are still at their default empty values -
				// we never overwrite data the user has already entered.
				bool isNew = ( _source == null || _source.Count == 0 )
				...
				switch ( value )
				{
					case ActionType.Extract:
						_source = new List<string> { @"<<modDirectory>>\" };
						_overwrite = true;
						break;

					case ActionType.TSLPatcher:
						_source = new List<string> { @"<<modDirectory>>\" };
						_destination = @"<<kotorDirectory>>";
						_overwrite = true;
						break;

					case ActionType.Move:
						_source = new List<string> { @"<<modDirectory>>\" };
						_destination = @"<<kotorDirectory>>\Override";
						_overwrite = true;
						break;
				...
				// Notify the GUI that these fields changed so bindings update.
		...
		public async Task<ActionExitCode> ExecuteTSLPatcherAsync()
		{
			try
			{
				foreach ( string t in RealSourcePaths )
				{
					...
					try
					{
						// VerifyInstall() reads TSLPatcher's installlog.rtf/.txt from the
						// tslpatchdata folder.  HoloPatcher does NOT write such a file there
						// (it writes its own backup log elsewhere), so VerifyInstall() always
						// throws FileNotFoundException when HoloPatcher is in use, causing KMS
						// to return TSLPatcherLogNotFound even on a perfect install.
						//
						// HoloPatcher's process exit code is authoritative: 0 = success,
						// non-zero = failure.  We already checked exitCode != 0 above, so if
						// we reach this point with HoloPatcher the install succeeded – skip
						// the TSLPatcher-specific log check entirely.

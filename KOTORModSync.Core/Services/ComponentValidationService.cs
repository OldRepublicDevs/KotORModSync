...
.ConfigureAwait( false );
				...
				shouldTryFallback)
			{
				...
				try
				{
					...
					if (fallbackMissingFileIssues.Count == 0 && fallbackExitCode == ModComponent.InstallExitCode.Success)
					{
						...
						List<ModComponent> missingDependencies = await FindMissingDependenciesAsync( component, modArchiveDirectory, MainConfig.AllComponents, cancellationToken ).ConfigureAwait( false );
				...
				foreach (string failedPattern in failedPatterns)
				{
					...
					Instruction extractInstruction = component.Instructions
						.FirstOrDefault( i => i.Action == Instruction.ActionType.Extract &&
											 i.Source.Exists( s => FileMatchesPattern( expectedArchive,
			...
			CancellationToken cancellationToken = default )
		{
			...
			int sourceIndex = extractInstruction.Source.FindIndex( s => string.Equals( ExtractFilenameFromSource( s ), oldArchiveName, StringComparison.Ordinal ) );
			if (sourceIndex >= 0)
			{

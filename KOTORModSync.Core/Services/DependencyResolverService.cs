...
namespace KOTORModSync.Core.Services
{
	...
	public static class DependencyResolverService
	{
			...
			bool ignoreErrors = false)
		{
			...
			var validationErrors = ValidateDependencies(components, componentDict);
			...
			var dependencyGraph = BuildDependencyGraph(components, componentDict);

			// Detect circular dependencies
			var circularDependencies = DetectCircularDependencies(dependencyGraph, componentDict);
			...
			try
			{
				var orderedComponents = PerformTopologicalSort(dependencyGraph, componentDict);
			...
			Dictionary<Guid, ModComponent> componentDict)
		{
			...
			foreach (var component in components)
			{
				...
				foreach (var beforeGuid in component.InstallBefore)
				{
				...
				foreach (var afterGuid in component.InstallAfter)
				{
			...
			Dictionary<Guid, ModComponent> componentDict)
		{
			...
			foreach (var componentGuid in graph.Keys)
			{
				if (!visited.Contains(componentGuid))
				{
					var cycle = DetectCycleDFS(componentGuid, graph, visited, recursionStack);
					if (cycle != null)
					{
						var cycleNames = cycle.Select(guid =>
							componentDict.TryGetValue(guid, out var comp) ? comp.Name : guid.ToString()).ToList();
			...
			HashSet<Guid> recursionStack)
		{
			...
			if (graph.ContainsKey(current))
			{
				foreach (var neighbor in graph[current])
				{
					...
					{
						var cycle = DetectCycleDFS(neighbor, graph, visited, recursionStack);
						if (cycle != null)
							return cycle;
					}
			...
			List<ModComponent> result)
		{
			...
			if (componentDict.TryGetValue(current, out var component))
			{

...
.ConfigureAwait( false );
				...
				Task loginTimeoutTask = Task.Delay( TimeSpan.FromSeconds( TimeoutSeconds ), cancellationToken );
...

		{
			...
			try
			{
				...
				try
				{
					...
					Task preLogoutTimeout = Task.Delay( TimeSpan.FromSeconds( 10 ), cancellationToken );
...
.ConfigureAwait( false );
				...
				Task<INode> getNodeTask = _client.GetNodeFromLinkAsync( new Uri( processedUrl ) );
				Task nodeTimeoutTask = Task.Delay( TimeSpan.FromSeconds( TimeoutSeconds ), cancellationToken );
...
.ConfigureAwait( false );
				...
				Progress<double> megaProgress = new Progress<double>( percent =>
				{
				...
				Task downloadTimeoutTask = Task.Delay( TimeSpan.FromSeconds( downloadTimeoutSeconds ), cancellationToken );
				...
				Task logoutTimeoutTask = Task.Delay( TimeSpan.FromSeconds( 10 ), cancellationToken );
			...
			finally
			{
				try
				{
					...
					Task finalLogoutTimeout = Task.Delay( TimeSpan.FromSeconds( 5 ) );
		...
		public async Task<Dictionary<string, object>> GetFileMetadataAsync( string url, CancellationToken cancellationToken = default )
		{
			...
			try
			{
				...
				try
				{
					...
					Task getInfoTimeoutTask = Task.Delay( TimeSpan.FromSeconds( TimeoutSeconds ), cancellationToken );

...
.ConfigureAwait( false );
				...
				HttpResponseMessage response = await MakeApiRequestAsync( request, cancellationToken )
...
.ConfigureAwait( false );
				...
				JObject filesData = Newtonsoft.Json.Linq.JObject.Parse( jsonResponse );
				List<JObject> files = filesData["files"]?.ToObject<List<Newtonsoft.Json.Linq.JObject>>();
			...
			try
			{
				...
				foreach (JObject file in files)
				{
					...
					try
					{
						...
						JObject downloadData = Newtonsoft.Json.Linq.JObject.Parse( downloadJson );

						JArray uriArray = downloadData["download_links"] as Newtonsoft.Json.Linq.JArray;
		...
		public async Task<Dictionary<string, object>> GetFileMetadataAsync( string url, CancellationToken cancellationToken = default )
		{
			...
			try
			{
				...
				if (fileId != null)
				{
					...
					{
						string content = await response.Content.ReadAsStringAsync().ConfigureAwait( false );
						JObject fileData = Newtonsoft.Json.Linq.JObject.Parse( content );
						...
						if (fileData["size_kb"] != null)
						{
							...
							if (sizeKbToken.Type == Newtonsoft.Json.Linq.JTokenType.Integer)
								sizeKb = sizeKbToken.Value<long>();
							else if (sizeKbToken.Type == Newtonsoft.Json.Linq.JTokenType.Float)
						...
						if (fileData["uploaded_timestamp"] != null)
						{
							...
							if (timestampToken.Type == Newtonsoft.Json.Linq.JTokenType.Integer)

						...
						using ( var errorWaitHandle = new AutoResetEvent(initialState: false) )
						{
							...
							process.ErrorDataReceived += (sender, e) =>
							{
								try
								{
									...
									// HoloPatcher v1.6+ routes all of its logging to stderr with ANSI
									// colour codes (e.g. "\x1b[0;37m") and Python-style level prefixes
									// (INFO/DEBUG/WARNING/ERROR).  Blindly calling LogErrorAsync here
									// tagged every INFO line as "[Error]" in the KMS log, making a
									// successful install look like a failure.
									//
									// Fix: strip ANSI escapes, then decide the KMS log level from the
									// content.  Only use LogErrorAsync when the line genuinely carries
									// an error marker.
									...
									// netstandard2.0 lacks the StringComparison overload of Contains/StartsWith,
									// so use IndexOf(..., StringComparison) which is available on all targets.
									if ( string.IsNullOrWhiteSpace(stripped) )
									{
										// nothing useful – skip
									}
									else if ( stripped.IndexOf("[Error]", StringComparison.OrdinalIgnoreCase) >= 0
										|| stripped.IndexOf("ERROR(", StringComparison.OrdinalIgnoreCase) == 0 )
									{
										_ = Logger.LogErrorAsync(stripped);
									}
									else if ( stripped.IndexOf("[Warning]", StringComparison.OrdinalIgnoreCase) >= 0
										|| stripped.IndexOf("WARNING(", StringComparison.OrdinalIgnoreCase) == 0 )
									{
										_ = Logger.LogWarningAsync(stripped);
									}
									else if ( stripped.IndexOf("[Verbose]", StringComparison.OrdinalIgnoreCase) >= 0
										|| stripped.IndexOf("DEBUG(", StringComparison.OrdinalIgnoreCase) == 0 )
									{
										_ = Logger.LogVerboseAsync(stripped);
									}
									else
									{
										_ = Logger.LogAsync(stripped);
									}
								}
								catch ( Exception exception )
								{

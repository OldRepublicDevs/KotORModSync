	...
	public partial class DownloadLinksControl : UserControl
	{
        ...
        private async void ResolveFilenames_Click(object sender, RoutedEventArgs e)
		{
			...
			try
			{
				...
				{
					await Logger.LogWarningAsync("[DownloadLinksControl] Download manager not initialized").ConfigureAwait(true);
					return;
				}

				await Logger.LogAsync($"[DownloadLinksControl] Resolving filenames for URL: {url}").ConfigureAwait(true);
				...
				using (var cts = new CancellationTokenSource(TimeSpan.FromMinutes(5)))
				{
					...
					{
						await Logger.LogAsync($"[DownloadLinksControl] Resolved {filenames.Count} filename(s) for URL: {url}").ConfigureAwait(true);
					...
					else
					{
						...
						var expectedFilenames = component?.ModLinkFilenames.TryGetValue(url, out var filenameDict) == true
							...
							: "none";
						await Logger.LogWarningAsync($"[DownloadLinksControl] Failed to resolve filenames for URL: {url}{componentInfo} Expected filename(s): {expectedFilenames}").ConfigureAwait(true);
					}
				}
			}
			catch (Exception ex)
			{
				var component = GetCurrentComponent();
				string componentInfo = component != null ? $" [Component: '{component.Name}']" : "";
				await Logger.LogExceptionAsync(ex, $"Error resolving filenames for URL: {button.Tag}{componentInfo}").ConfigureAwait(true);
			}
		}
		...
		/// </summary>
		private static void SyncDownloadLinksToModLinkFilenames(ModComponent component, List<string> urlList)
		{
		...
		private async void AddFilename_Click(object sender, RoutedEventArgs e)
		{
			...
			try
			{
				...
				await dialog.ShowDialog(this.GetVisualRoot() as Window).ConfigureAwait(true);
				...
				{
					await Logger.LogWarningAsync($"[DownloadLinksControl] Filename already exists: {filename}").ConfigureAwait(true);
					return;
				}

				// Add with null (auto-detect) by default
				filenameDict[filename] = null;
				await Logger.LogVerboseAsync($"[DownloadLinksControl] Added filename '{filename}' to URL '{url}'").ConfigureAwait(true);
			...
			{
				await Logger.LogExceptionAsync(ex, "Error adding filename").ConfigureAwait(true);
			}
		}
		...
		private async void RemoveFilename_Click(object sender, RoutedEventArgs e)
		{
			...
			if (component is null)
				return;
			if (
				component.ModLinkFilenames.TryGetValue(url, out Dictionary<string, bool?> filenameDict)
				&& filenameDict.Remove(filename)
			)
			{
				await Logger.LogVerboseAsync($"[DownloadLinksControl] Removed filename '{filename}' from URL '{url}'").ConfigureAwait(true);
				UpdateFilenamePanelForUrl(url);
			}
		}
		...
		private async void DownloadMod_Click(object sender, RoutedEventArgs e)
		{
			...
			try
			{
				await Logger.LogVerboseAsync($"[DownloadLinksControl] Download button clicked for: {component.Name}").ConfigureAwait(true);
			...
			{
				await Logger.LogExceptionAsync(ex, $"Error downloading mod from DownloadLinksControl: {component.Name}").ConfigureAwait(true);
			}
		}
	}
}
	...
	public partial class MainWindow
	{
		...
		public async Task StartInstallWizardAsync()
		{
			try
			{
				...
				await Logger.LogVerboseAsync("[MainWindow] Starting wizard-based installation").ConfigureAwait(false);
				...
				await Dispatcher.UIThread.InvokeAsync(async () =>
				{
					...
					bool? result = await wizard.ShowDialog<bool?>(this).ConfigureAwait(true);

					if (wizard.InstallationCompleted && result == true)
					{
						await Logger.LogAsync("Installation wizard completed successfully").ConfigureAwait(false);

						// Update UI to reflect installation completion
						await UpdateStepProgressAsync().ConfigureAwait(true);
					...
					{
						await Logger.LogAsync("Installation wizard was cancelled by user").ConfigureAwait(false);
			...
			{
				await Logger.LogExceptionAsync(ex, customMessage: "Error in wizard-based installation").ConfigureAwait(false);

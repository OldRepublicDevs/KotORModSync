...
using static KOTORModSync.Core.Utility.UtilityHelper;
	...
	public class DownloadOrchestrationService
	{
		...
		public async Task StartDownloadSessionAsync( Action onScanComplete, bool sequential = true )
		{
			try
			{
				...
				await Logger.LogVerboseAsync( $"[DownloadOrchestration] Using download timeout: {timeoutMinutes} minutes" ).ConfigureAwait( false );
...
.ConfigureAwait( false );
							...
							case DownloadControlAction.Stop:
								...
								await HandleStopDownloadAsync( args.Progress, downloadManager ).ConfigureAwait( false );
								break;
 ...
 progressWindow.Show()
					...
					catch (Exception ex)
					{
						...
						await Dispatcher.UIThread.InvokeAsync( async () =>
						{
							...
							await InformationDialog.ShowInformationDialogAsync( _parentWindow,
								$"An error occurred while downloading mods:{Environment.NewLine}{Environment.NewLine}{ex.Message}" ).ConfigureAwait( false );
						} ).ConfigureAwait( false );
					}
				}) );
			}
			catch (Exception ex)
			{
				...
				{
					await InformationDialog.ShowInformationDialogAsync( _parentWindow,
						$"An error occurred while starting downloads:{Environment.NewLine}{Environment.NewLine}{ex.Message}" ).ConfigureAwait( false );
				} ).ConfigureAwait( false );
			}
		}

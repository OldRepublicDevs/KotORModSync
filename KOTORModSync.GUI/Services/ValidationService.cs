		...
		public async Task AnalyzeValidationFailures(List<Dialogs.ValidationIssue> modIssues, List<string> systemIssues)
		{
			try
			{
				...
				foreach ( ModComponent component in _mainConfig.allComponents.Where(c => c.IsSelected) )
				{
					...
					bool componentValid = await Core.Services.ComponentValidationService.ValidateComponentFilesExistAsync(component);
					if ( !componentValid )
					{

						List<string> missingFiles = await Core.Services.ComponentValidationService.GetMissingFilesForComponentAsync(component);

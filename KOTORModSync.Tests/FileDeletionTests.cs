...
namespace KOTORModSync.Tests
{
	...
	public class FileDeletionTests
	{
		...
		private async Task<(VirtualFileSystemProvider virtualProvider, string realSource, string realDest)> RunBothProviders( List<Instruction> instructions, string sourceDir, string destDir )
		{
			...
			await virtualProvider.InitializeFromRealFileSystemAsync( virtualSource ).ConfigureAwait( false );
			...
			_ = await virtualComponent.ExecuteInstructionsAsync( virtualComponent.Instructions, [virtualComponent], CancellationToken.None, virtualProvider ).ConfigureAwait( false );
			...
			_ = await realComponent.ExecuteInstructionsAsync( realComponent.Instructions, [realComponent], CancellationToken.None, realProvider ).ConfigureAwait( false );
		...
		private async Task RunDeleteDuplicateFile( string directory, string fileExtension, List<string>? compatibleExtensions = null )
		{
			...
			var (virtualProvider, realSource, realDest) = await RunBothProviders( instructions, directory, directory ).ConfigureAwait( false );
